...
static const char* event_type_to_string(mysql_authorization_event_subclass_t event_type) {
  switch (event_type) {
...
static mysql_authorization_result_t simple_authorization_check(
  ...
  const char* result_str = (result == MYSQL_AUTHORIZATION_GRANT) ? "GRANT" :
                          (result == MYSQL_AUTHORIZATION_DENY) ? "DENY" : "IGNORE";
...
static int simple_authorization_deinit(MYSQL_PLUGIN plugin_info [[maybe_unused]]) {          
...
static SYS_VAR *simple_authorization_system_vars[] = {
  MYSQL_SYSVAR(allow_user),
  MYSQL_SYSVAR(allow_db),
  MYSQL_SYSVAR(mode),
  nullptr
};
